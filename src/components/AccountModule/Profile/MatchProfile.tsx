...
import { GetProfileRequest } from "../../../network/NetworkRequests"
...
import {
  ...
  spotifyTrackWrapper,
} from "./SpotifyComponentHelpers/spotifyComponentHelpers"
import {
  profilePicWrapper,
  facebookDetailsWrapper,
} from "./FacebookComponentHelpers/facebookComponentHelpers"
  ...
  function spotifyWrapper() {
    return (
      <div>
        {" "}
        <hr />{" "}
        <Card key="spotify" style={{ width: "100%", padding: "2em" }}>
          <Card.Title>Spotify</Card.Title>
          <hr />
          {account.spotify.artists.length > 0
            ? spotifyArtistWrapper(account)
            : null}
          {account.spotify.tracks.length > 0
            ? spotifyTrackWrapper(account)
            : null}
        </Card>
      </div>
    )
